...
export default function LoginPage() {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setError('Demo login failed. Starting local demo workspace...');
      router.push('/dashboard');
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#090d16] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Grounded RAG study copilot with pgvector embeddings
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        <div className="bg-white dark:bg-[#0c121e] py-8 px-6 sm:px-8 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-xl space-y-6">
          {/* Instant 1-Click Demo Login Banner for Recruiters */}
          <div className="p-4 rounded-2xl bg-gradient-to-r from-sky-500/10 via-indigo-500/10 to-purple-500/10 border border-sky-500/20 dark:border-sky-500/15 text-center space-y-2.5">
            <div className="flex items-center justify-center gap-1.5 text-xs font-bold text-sky-600 dark:text-sky-400">
              <Sparkles className="w-4 h-4" />
              <span>For Interviewers & Portfolio Reviewers</span>
            </div>
            <p className="text-[11px] text-slate-600 dark:text-slate-400">
              Skip registration! Explore pre-loaded documents, grounded RAG chats, quizzes & flashcards instantly.
            </p>
            <button
              onClick={handleDemoLogin}
              disabled={demoLoading}
              className="w-full py-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white text-xs font-bold shadow-md shadow-sky-500/20 transition-all hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2"
            >
              {demoLoading ? (
